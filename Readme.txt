...
1)Create two instances  on GCP 	 with 7.5 GB ram and 4 CPUS  and configure  to make ssh connection to instances or install gcloud utility  to make ssh coonection to instance . 
...
2) Download  jdk-7u80-linux-x64.tar.gz ,  apache-flume-1.7.0-bin.zip , apache-hive-1.2.1-bin.tar.gz   from internnet on local machine
...
4)hadoop version 2.7.3  on  master node  
 	...
 	*)Untar the Hadoo-2.7.3 tar file on master node ;
	*)Add master and slave node in /etc/hosts file
     		10.142.0.2     master
			10.142.0.3   slave
			
	*)Make the password less connection between both of the system
	*)Set  Hadoop path and Hadoop Home in ./bashrc  file   for   setting environment variable  in both of the system.
   ...
   *  Untar the Hadoo-2.7.3 tar file on slave node   &  copy  core-site.xml,hdfs-site.xml,Yarn-site.xml,mapred-site.xml   in   /home/shashi/hadoop-2.7.3/etc/hadoop  path  on second instance (35.185.89.152 ) 
   *  copy   ~/.bashrc   in    $HOME path  on second instance (35.185.89.152 ) 

    * Run the below command on first instnace   35.185.67.36 
...
7)Configuration  of each tool 
		...
		*) Script which runs  for Rest service which helps to get the  number  twits  based on  id of user.
		 ...
		 *) Script which runs  for Rest service which helps to get the  number of retweet_count  based on  id of user.
          ...
          *)Eclipse configuration  & created the  dynmaic web project and  developed  the rest services  for each operation 
